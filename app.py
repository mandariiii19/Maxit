...
def getboard():
    """
    создаём функцию для отображения игрового поля
    """
    for row in board:
        print(" | ".join(str(x) if x is not None else " " for x in row))
        print("-" * 9)
        
def avail_moves(player, pos):
    """
    создаем функцию,которая будет определять доступные ходы игроков, она принимает параметры, номер игрока и позицию
    """
    moves = []
